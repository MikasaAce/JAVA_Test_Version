    ...
    'javascript': [
        {
            ...
            ''',
            'pattern': r'^(cookie|document|cookies|window|globalThis|response|res)$',
            'property_pattern': r'^(set|append|add|push)$',
        ...
        },
        {
            ...
            ''',
            'pattern': r'^(document|cookie|window|globalThis|response|res)$',
            'property_pattern': r'^(cookie|cookies)$',
        ...
        },
        {
            ...
            ''',
            'pattern': r'^(setCookie|setcookie|cookie|setHttpOnly|setSecure|setHeader|writeHead|append)$',
...
# 常见HTTP响应头设置函数
RESPONSE_HEADER_FUNCTIONS = {
    'setHeader', 'writeHead', 'append', 'set'
}


def detect_js_setting_vulnerabilities(code, language='javascript'):
    """
        ...
        try:
            ...
            for node, tag in captures:
                if tag == 'object' or tag == 'func_name':
                    ...
                    current_capture['args'] = node.text.decode('utf8')
    ...
    try:
        ...
        for node, tag in captures:
            if tag == 'key':
                ...
                # 检查是否为HTTPOnly相关的键
                if current_pair['key'].lower() in ['httponly', 'http-only']:
    ...
    for cookie_op in cookie_operations:
        ...
        # 检查是否是响应头设置函数
        is_response_header = (
                cookie_op['property'] in RESPONSE_HEADER_FUNCTIONS or
                cookie_op['object'] in ['response', 'res'] and
                cookie_op['property'] in ['setHeader', 'writeHead', 'append']
        )

        # 对于响应头设置，检查是否设置了Set-Cookie头
        if is_response_header:
            # 检查参数中是否包含Set-Cookie
            args = cookie_op.get('args', '')
            if 'set-cookie' in args.lower() or 'cookie' in args.lower():
                # 检查是否包含httponly
                if 'httponly' in args.lower() or 'http-only' in args.lower():
                    # 检查值是否为真
                    if re.search(r'httponly\s*[:=]\s*true', args, re.IGNORECASE):
                        has_proper_httponly = True
                    elif re.search(r'httponly', args, re.IGNORECASE):
...
http.createServer((req, res) => {
    ...
    res.writeHead(200, {
        'Set-Cookie': ['user=john; Secure', 'token=def456; HttpOnly=false']
    });
});
...
const app = express();
app.use((req, res) => {
    res.cookie('session', 'value', { secure: true }); // 缺少httpOnly
    res.append('Set-Cookie', 'token=value; Secure'); // 缺少httpOnly
});
"""
